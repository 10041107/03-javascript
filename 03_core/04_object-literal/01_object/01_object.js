...
프로피터 값이 함수일 경우 메소드(method)라고 부른다.

객체 생성 방법
자바,c++과 같은 클래스 기반의 객체 지향 언어는 클래스를 사전에 정의하고 필요한 시점에 new연산자와 함께 생성자를 호출하고
인스턴스를 생성하는 방식으로 객체를 생성한다. 자바스크립트는 프토토타입 기반 객체지향 언어ㅏ로 클래스 기반 객체지향 언어와는 달리
다양한 객체 생성 방법을 지원한다.
