...
개발자가 의도적으로 값의 타입을 변환하는 것을 명시적 타입 변환(exclicit coercion)이라고 하며
자바스크립트 엔진에 의해 암묵적으로 타입이 자동 변환되는 것을 암묵적 타입 변환(implicit coercionk)이라고 한다.
명시적으로 타입 변환은 코드에서 드러나지만 암묵적인 타입 변환은 드러나지 않으므로
타입 변환 된 결과를 예측할 수 있어야 오류를 방지할 수 있다.

*/

//01 convert-to-string(문자열 타입 변환)
